...
// Demographic + health controls + provider
foreach outcome of local outcomes{
	capture noisily {
	  reghdfe `outcome' i.recip_race_g hcc*, ///
			   absorb(recip_age_g recip_gender_g recip_zip elig_cat state attr_prov) ///
			   vce(robust)
	  ...
	  local names : colfullnames e(b)
		foreach var of local names{
		  ...
		  file write myfile "demo_health_prov" _tab "`outcome'" _tab ("`var'") ///
